...
// These two functions deal with the "internal" IOS subsystems
// that are used by default by libogc
// Other stuff should be inited by the user and deinited by the
// exit callbacks. The user is also responsible for deiniting
// other stuff before an IOS reload and reiniting them after.
s32 __IOS_InitializeSubsystems(void)
{
	s32 res;
	s32 ret = 0;
#ifdef DEBUG_IOS
	exiPrintf("IOS Subsystem Init\n");
#endif
	res = __ES_Init();
	if(res < 0) {
		ret = res;
#ifdef DEBUG_IOS
		exiPrintf("ES Init failed: %d\n",ret);
#endif
	}
	res = __STM_Init();
	if(res < 0) {
		ret = res;
#ifdef DEBUG_IOS
		exiPrintf("STM Init failed: %d\n",ret);
#endif
	}
#ifdef DEBUG_IOS
	exiPrintf("IOS Subsystem Init Done: %d\n",ret);
#endif
	return ret;
}
...
s32 __IOS_ShutdownSubsystems(void)
{
	...
	if(res < 0) ret = res;
#ifdef DEBUG_IOS
	exiPrintf("IOS Subsystem Close Done: %d\n",ret);
#endif
	return ret;
}
...
s32 IOS_GetPreferredVersion(void)
{
	...
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf(" GetNumTitles failed: %d\n",res);
#endif
		return res;
	}
#ifdef DEBUG_IOS
	exiPrintf(" %d titles on card:\n",count);
#endif
	titles = iosAlloc(__ios_hid, sizeof(u64)*count);
	if(!titles) {
		exiPrintf(" iosAlloc titles failed\n");
		return -1;
	}
	res = ES_GetTitles(titles, count);
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf(" GetTitles failed: %d\n",res);
#endif
...
s32 __IOS_LaunchNewIOS(int version)
{
	...
	if(oldversion>0) exiPrintf("Current IOS Version: IOS%d\n",oldversion);
#endif

	titleID |= version;
#ifdef DEBUG_IOS
	exiPrintf("Launching IOS TitleID: %016llx\n",titleID);
#endif

	res = ES_GetNumTicketViews(titleID, &numviews);
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf(" GetNumTicketViews failed: %d\n",res);
#endif
		return res;
	}
	if(numviews > 4) {
		exiPrintf(" GetNumTicketViews too many views: %u\n",numviews);
		return IOS_ETOOMANYVIEWS;
	}
	res = ES_GetTicketViews(titleID, views, numviews);
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf(" GetTicketViews failed: %d\n",res);
#endif
		return res;
	}
	...
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf(" LaunchTitleBackground failed: %d\n",res);
#endif
		return res;
	}
	...
	exiPrintf("IOS loaded: IOS%d v%d.%d\n", v >> 16, (v >> 8) & 0xff, v & 0xff);
#endif

#ifdef DEBUG_IOS
	exiPrintf("Waiting for IPC ...\n");
#endif
...
s32 __attribute__((weak)) __IOS_LoadStartupIOS(void)
{
	...
	if(version < 0) {
#ifdef DEBUG_IOS
		exiPrintf("GetPreferredVersion failed: %d\n",version);
#endif
		__ES_Close();
		return version;
	}
#ifdef DEBUG_IOS
	exiPrintf("Loading startup IOS: %d\n",version);
#endif
...
s32 IOS_ReloadIOS(int version)
{
...
#ifdef DEBUG_IOS
	exiPrintf("Reloading to IOS%d\n",version);
#endif

	res = __IOS_ShutdownSubsystems();
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf("__IOS_ShutdownSubsystems failed: %d\n", res);
#endif
		ret = res;
	}
	...
	if(res < 0) {
		...
		if(res < 0) {
#ifdef DEBUG_IOS
			exiPrintf("__IOS_LaunchNewIOS failed: %d\n", res);
#endif
			ret = res;
			__ES_Close();
		}
	...
	if(res < 0) {
#ifdef DEBUG_IOS
		exiPrintf("__IOS_InitializeSubsystems failed: %d\n", res);
#endif
		ret = res;
	}
