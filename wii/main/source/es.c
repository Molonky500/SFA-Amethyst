...
s32 __ES_Init(void)
{
	...
	if(__es_hid < 0 ) {
		...
		if(__es_hid < 0) {
			exiPrintf(" *** ERROR *** %s heap create failed: %d\n",
				__FUNCTION__, __es_hid);
			return __es_hid;
		}
	}
	...
	if (__es_fd < 0) {
		...
		if(ret<0) {
			exiPrintf(" *** ERROR *** %s open failed: %d\n",
				__FUNCTION__, ret);
			return ret;
		}
		__es_fd = ret;
...
static int _ES_close_r (struct _reent *r, void *fd) {
	...
	//OSDestroyMutex(&file->mutex); //no such function
	return 0;
}
