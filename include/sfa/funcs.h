...
GLOBALFN BOOL audioStopSound(uint id);
...
GLOBALFN void GXSetLineWidth(u8 width, GXTexOffset texOffsets);
...
GLOBALFN void GXSetPointSize(u8 pointSize, GXTexOffset texOffsets);
...
GLOBALFN RomCurve* RomCurve_getById(int id);
...
GLOBALFN void ScreenTransition_screenFade(void /*three unused params*/);
...
GLOBALFN float Vec_distance(vec3f *a, vec3f *b);
GLOBALFN double Vec_length(vec3f *vec);
GLOBALFN void Vec_multByMtx (Mtx * mtx, vec3f * vIn, vec3f * vOut);
GLOBALFN void Vec_multByMtx2(Mtx * mtx, vec3f * vIn, vec3f * vOut);
GLOBALFN void Vec_scale(double scale, vec3f *in, vec3f *out);
GLOBALFN float Vec_xzDistance(vec3f *a, vec3f *b);
